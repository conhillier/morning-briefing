...
def select_cfo_items(items, previous_headlines):
    ...
    user = (
        ...
        f"YOUR READER IS: a CFO running a UK OPERATING / INDUSTRIAL business based in "
        f"Scotland. They sell products or services to customers. They are NOT a bank, "
        f"NOT a tax adviser, NOT an audit firm, NOT a charity, NOT a law firm, NOT an "
        f"insurer. They employ staff, pay corporation tax, file at Companies House, "
        f"reimburse mileage, and care about borrowing costs.\n\n"
        f"Pick up to {CFO_TARGET_COUNT} items that would CHANGE SOMETHING this CFO "
        f"actually does. Be ruthless. The acid test: 'If I told my finance team about "
        f"this on Monday morning, would it change a policy, a calculation, a filing, "
        f"a deadline, a rate they apply, or a reimbursement they make?'\n\n"
        f"INCLUDE (these CHANGE WORK for an operating business):\n"
        f"- Corporation tax rate, banding, or relief changes\n"
        f"- Capital allowances: AIA, full expensing, super-deduction extensions\n"
        f"- R&D tax credit scope, rates, or qualifying-expenditure changes\n"
        f"- HMRC Approved Mileage rates (AMAP) - the rate they reimburse staff at\n"
        f"- Advisory Fuel Rates (AFR) - quarterly company-car rates\n"
        f"- Employer NI rates, NI thresholds (Secondary, Apprentice, Veterans)\n"
        f"- VAT registration threshold or VAT scheme scope changes\n"
        f"- Statutory rates: National Minimum Wage, Living Wage, SSP, statutory parental pay\n"
        f"- Pension auto-enrolment thresholds (earnings trigger, qualifying earnings)\n"
        f"- IR35 / off-payroll working rule changes\n"
        f"- Making Tax Digital scope changes (which businesses must comply, deadlines)\n"
        f"- Companies House filing requirement or penalty changes (annual accounts, CS01)\n"
        f"- Companies Act audit-threshold changes (turnover/balance-sheet/employee bands)\n"
        f"- FRS 102 / FRS 105 / UK GAAP amendments affecting reporting\n"
        f"- Bank of England base rate decisions (directly affects borrowing cost)\n"
        f"- Business rates relief schemes (especially Scotland)\n"
        f"- Apprenticeship Levy changes\n"
        f"- Major Budget announcements with specific business impact\n\n"
        f"EXCLUDE - these do NOT affect an industry CFO even if framed as 'business news':\n"
        f"- Tax-adviser, agent, or accountancy-firm regulation (your reader HIRES tax advisers, "
        f"they don't operate as one)\n"
        f"- Bank-only reforms: PRA / FCA prudential rules, ring-fencing, MREL, capital requirements\n"
        f"- Insurance-only reforms: Solvency UK, conduct rules for insurers\n"
        f"- Pension-fund/trustee-only regulation (TPR rules for schemes themselves)\n"
        f"- Charity Commission guidance\n"
        f"- Personal-tax news: Child Benefit, Child Trust Funds, Self Assessment for individuals\n"
        f"- Ministerial speeches without a specific rate/rule announced\n"
        f"- Statistical notices, UAT environments, taxonomy updates, consultation launches "
        f"with no rule yet\n"
        f"- Recruitment / appointment news\n"
        f"- Banknote design, branch openings, financial-inclusion campaigns\n\n"
        f"If NOTHING in the pool genuinely qualifies, return an empty array. Most days "
        f"that's the right answer. DO NOT PAD.\n"
        f"{dedup_block}\n\n"
        f"=== POOL ===\n{digest}\n\n"
        f'Return ONLY JSON: {{"cfo_items": [<index>, ...]}}'
    )
    system = (
        "You are a senior UK Chartered Accountant who has advised CFOs at operating "
        "businesses (manufacturing, distribution, services) for 20 years. You do NOT "
        "work for a bank or a tax practice. You output only valid JSON and you err on "
        "the side of returning fewer items - or none at all."
    )
